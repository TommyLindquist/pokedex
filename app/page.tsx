...
import PokeeCardWrapper from "@/components/pokee-card-wrapper";
...
export default async function Home({
  ...
  const { query = "", qs = [] } = await searchParams;

  const names = (Array.isArray(qs) && qs.length === 3
    ? [...qs, (await getRandomNames(1)).splice(0, 1)]
    : await getRandomNames(4)) as string[];
  ...
  return (
    <main>
      ...
      <section className="relative flex flex-col items-center gap-4 bg-gradient-to-br [background-image:linear-gradient(-10deg,_#C97FE4,_#AECDF6)] p-14">
        ...
        <RandomButton qs={names.slice(0, 3)} />

      </section>
      <Search placeholder="Search for pokemon..." className="m-1" />
      {query && (
        <Suspense
          key={query as string}
          fallback={
          ...
          <PokeeCardWrapper query={query as string} />
        </Suspense>
      )}

      <section className="flex flex-wrap justify-center gap-4 bg-gray-100 p-8">
        <Suspense
          // turned of now, because didn´t like the flickering much ...
          // key={names[3]}
          fallback={
            <div className="animate-pulse w-58 h-65 bg-neutral-400/70 border rounded border-neutral-400 p-4">
              Loading...
